...
function getPlatformPresets() {
	...
	console.log(`[平台预设调试] 内置默认平台:`, wanted);
	try {
		const raw = localStorage.getItem('priceCalculatorPlatformPresets');
		console.log(`[平台预设调试] localStorage原始数据:`, raw);
		if (raw) {
			const list = JSON.parse(raw);
			console.log(`[平台预设调试] 解析后的localStorage数据:`, list);
			if (Array.isArray(list)) {
				...
				if (changed) {
					console.log(`[平台预设调试] 检测到变更，保存更新后的数据:`, list);
					savePlatformPresets(list);
				}
				console.log(`[平台预设调试] 最终返回平台列表:`, list);
				return list;
			}
		}
	} catch (error) {
		console.error(`[平台预设调试] 解析localStorage出错:`, error);
	}
	// 没有存储时返回内置默认
	console.log(`[平台预设调试] 使用内置默认平台列表:`, wanted);
	return wanted;
}
...
function getPlatformRateByName(name) {
	...
	console.log(`[平台佣金率调试] 查找平台: "${name}", 可用平台列表:`, list);
	const item = list.find(x => x && x.name === name);
	console.log(`[平台佣金率调试] 匹配结果:`, item);
	const result = item && isFinite(Number(item.rate)) ? Number(item.rate) : NaN;
	console.log(`[平台佣金率调试] 最终返回佣金率: ${result}`);
	return result;
}
	...
	const overallHtml = `
		<div style="margin-bottom:24px; padding:16px; background:#f8fafc; border:1px solid #e5e7eb; border-radius:8px;">
			...
			<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(200px, 1fr)); gap:16px;">
				...
				<div style="text-align:center; padding:12px; background:#fff; border:1px solid #e5e7eb; border-radius:6px; cursor:pointer; transition:background-color 0.2s;" 
					 onclick="showRiskProductsDetail()" 
...
// 显示风险商品详情弹窗
function showRiskProductsDetail() {
	// 获取当前商品数据（使用筛选后的数据或原始数据）
	const rows = catalogFilterState.filteredRows.length > 0 ? catalogFilterState.filteredRows : (catalogState.rows || []);
	
	if (rows.length === 0) {
		showToast && showToast('暂无商品数据');
		return;
	}
	...
	rows.forEach((row, index) => {
		...
		if (isRiskProduct) {
			...
			let riskLevel = '';
			let riskDescription = '';
			if (breakevenAdRate < 0.10) {
				riskLevel = '高风险';
				riskDescription = '保本广告占比低于10%，盈利能力极差';
			} else if (breakevenAdRate < 0.15) {
				riskLevel = '中高风险';
				riskDescription = '保本广告占比低于15%，盈利能力较差';
			} else {
				riskLevel = '中等风险';
				riskDescription = '保本广告占比低于21%，需要关注盈利能力';
			}
		...
		<div style="margin-bottom:12px; padding:12px; background:#fef2f2; border:1px solid #fecaca; border-radius:8px; border-left:4px solid #dc2626;">
			<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:8px;">
				...
				<div style="text-align:right;">
					<span style="padding:4px 8px; background:#dc2626; color:white; border-radius:12px; font-size:12px; font-weight:600;">${product.riskLevel}</span>
				</div>
			</div>
	...
	panel.innerHTML = `
		<div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:20px;">
			<div>
				<div style="font-weight:700; font-size:18px; color:#dc2626;">⚠️ 风险商品详情</div>
				<div style="font-size:14px; color:#6b7280; margin-top:4px;">共发现 ${riskProducts.length} 个风险商品（保本广告占比 < 21%）</div>
			</div>
	...
	// 绑定关闭事件
	panel.querySelector('#riskDetailClose').addEventListener('click', ()=>{ 
		try { document.body.removeChild(overlay); } catch(_){} 
	});
...
function showMetricTooltip(event, metricType, scope, data) {
	...
	if (metricType === 'markup') {
		...
		tooltipContent = `平均加价率 = 所有商品加价率的总和 ÷ 商品总数量
...
• 计算范围：${scope === 'overall' ? '整体' : scope}`;
		...
		tooltipContent = `平均退货率 = 所有商品退货率的总和 ÷ 商品总数量
...
• 计算范围：${scope === 'overall' ? '整体' : scope}`;
		...
		tooltipContent = `平均保本ROI = 所有商品保本ROI的总和 ÷ 商品总数量
...
• 计算范围：${scope === 'overall' ? '整体' : scope}`;
		...
		tooltipContent = `平均保本广告占比 = 所有商品保本广告占比的总和 ÷ 商品总数量
...
function showCatalogProfitScenario(row){
	...
	// 生成到手价推演矩阵HTML（保持原来表格样式，使用新的精确计算逻辑）
	const generateTakeHomePriceTableHtml = (costTable) => {
		...
		// 表格行：每行显示一个付费占比，每列显示对应目标利润率的到手价（保持原来样式，但使用新数据）
		const rows = adRates.map(adRate => {
			...
			const cells = targetProfitRates.map(targetRate => {
				...
				if (isNaN(takeHomePrice) || takeHomePrice <= 0) {
					return `<td style="padding:8px 10px;text-align:center;border-right:1px solid #f2f2f2;color:#9ca3af;">—</td>`;
				}
				
				// 计算对应的利润金额和保本ROI（使用新逻辑）
				let profitAmount = 0;
				let breakevenROI = '-';
				let breakevenAdRate = '-';
				...
				const calculationData = {
					...
					takeHomePrice: takeHomePrice
				};

				// 使用原来的tooltip格式，但包含新的计算数据
				...
				return `<td style="padding:8px 10px;text-align:center;border-right:1px solid #f2f2f2;color:#16a34a;font-weight:600;" 
					data-tooltip="${tooltip.replace(/"/g, '&quot;')}" data-calculation="${encodeURIComponent(JSON.stringify(calculationData))}">${isFinite(takeHomePrice) ? ('¥' + takeHomePrice.toFixed(2)) : '-'}</td>`;
			}).join('');
	...
	const html = `
		<div class="pv-mask">
			<div class="pv-modal">
				<div class="pv-hd">
					<div class="pv-title">利润推演 - ${row.name||''}（${row.sku||''}）<span style="color:#6b7280;font-size:0.9em;margin-left:8px;">${row.platform||''}</span><span style="color:#10b981;font-size:0.9em;margin-left:8px;">✓ 已统一计算逻辑</span><span style="color:#3b82f6;font-size:0.9em;margin-left:8px;">✓ 到手价推演已完善</span></div>
				...
				<div class="pv-body">
					...
					<div style="background:#f0f4ff; border:1px solid #3b82f6; border-radius:8px; padding:12px; margin-bottom:16px; color:#1e40af;">
						...
						<div style="font-size:13px; line-height:1.4;">
							本弹窗使用与"利润率计算"tab完全一致的计算逻辑，包括进项税抵扣计算方式（0.06/1.06），确保结果完全一致。<br>
							<strong>新增：</strong>到手价推演功能已完善，使用与"到手价推演"tab完全一致的精确计算逻辑，包括保本ROI和保本广告占比计算。<br>
							<strong>样式保持：</strong>保持原来的表格样式，确保界面一致性。
						</div>
					</div>
	...
	(function attachTooltipDelegation(){
		...
		const onOver = (e) => {
			...
			if (text) {
				// 检查是否有详细计算过程数据
				if (cell.hasAttribute('data-calculation')) {
					const calculationData = cell.getAttribute('data-calculation');
					showTakeHomeCalculationTooltip(e, text, calculationData);
				} else {
					showCatalogTooltip(text, e.clientX, e.clientY);
				}
			}
		};
		const onMove = (e) => {
    ...
    if (table) {
        ...
        table.addEventListener('mouseover', function(e) {
            if (e.target.hasAttribute('data-tooltip') && e.target.hasAttribute('data-calculation')) {
                showTakeHomeCalculationTooltip(e, e.target.getAttribute('data-tooltip'), e.target.getAttribute('data-calculation'));
...
function generateTakeHomePriceTableHtmlForExploration(returnRateResults, adRates, targetProfitRates, returnRate) {
    ...
    return `
        <div style="background:#fff; border:1px solid #e5e7eb; border-radius:12px; overflow-x:auto; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);" class="takehome-table-container">
            <table style="width:100%; min-width:600px; border-collapse:collapse; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;" class="takehome-result-table" id="takehomeExplorationTable">
                ${header}
                ${rows}
            </table>
        </div>
...
/**
 * 生成成本价推演浮窗HTML内容
 * @param {string} tooltipText 原始tooltip文本
 * @returns {string} 格式化的HTML内容
 */
function generateCostPriceTooltipHtml(tooltipText) {
    // 解析tooltip文本，提取关键信息
    const lines = tooltipText.split('\n');
    const info = {};
    
    // 添加调试信息
    console.log('解析tooltip文本:', tooltipText);
    
    lines.forEach((line, index) => {
        console.log(`第${index + 1}行: "${line}"`);
        
        // 基础参数行 - 包含目标利润率和推演成本价
        if (line.includes('目标利润率:') && line.includes('推演成本价:')) {
            const targetProfitPart = line.split('目标利润率:')[1].split('推演成本价:')[0].trim();
            const costPricePart = line.split('推演成本价:')[1].trim();
            info.targetProfitRate = targetProfitPart;
            info.costPrice = costPricePart;
            console.log('提取推演成本价:', costPricePart); // 添加调试信息
        }
        // 付费占比行 - 包含付费占比和开票成本
        else if (line.includes('付费占比:') && line.includes('开票成本:')) {
            const adRatePart = line.split('付费占比:')[1].split('开票成本:')[0].trim();
            const invoiceCostPart = line.split('开票成本:')[1].trim();
            info.adRate = adRatePart;
            info.invoiceCost = invoiceCostPart;
        }
        // 退货率行 - 包含退货率和总进货成本
        else if (line.includes('退货率:') && line.includes('总进货成本:')) {
            const returnRatePart = line.split('退货率:')[1].split('总进货成本:')[0].trim();
            const totalPurchasePart = line.split('总进货成本:')[1].trim();
            info.returnRate = returnRatePart;
            info.totalPurchaseCost = totalPurchasePart;
        }
        // 有效销售率行 - 包含有效销售率和商品进项税
        else if (line.includes('有效销售率:') && line.includes('商品进项税:')) {
            const effectiveRatePart = line.split('有效销售率:')[1].split('商品进项税:')[0].trim();
            const purchaseVATPart = line.split('商品进项税:')[1].trim();
            info.effectiveRate = effectiveRatePart;
            info.purchaseVAT = purchaseVATPart;
        }
        // 固定成本行 - 包含固定成本和计算利润
        else if (line.includes('固定成本:') && line.includes('计算利润:')) {
            const fixedCostsPart = line.split('固定成本:')[1].split('计算利润:')[0].trim();
            const profitPart = line.split('计算利润:')[1].trim();
            info.fixedCosts = fixedCostsPart;
            info.profit = profitPart;
        }
        // 分摊后成本行 - 包含分摊后成本和利润率
        else if (line.includes('分摊后成本:') && line.includes('利润率:')) {
            const allocatedCostsPart = line.split('分摊后成本:')[1].split('利润率:')[0].trim();
            const profitRatePart = line.split('利润率:')[1].trim();
            info.allocatedCosts = allocatedCostsPart;
            info.profitRate = profitRatePart;
        }
        // 广告费行 - 包含广告费和目标差异
        else if (line.includes('广告费:') && line.includes('目标差异:')) {
            const adCostPart = line.split('广告费:')[1].split('目标差异:')[0].trim();
            const targetDiffPart = line.split('目标差异:')[1].trim();
            info.adCost = adCostPart;
            info.targetDiff = targetDiffPart;
        }
        // 分摊广告费行 - 包含分摊广告费和理论到手价
        else if (line.includes('分摊广告费:') && line.includes('理论到手价:')) {
            const allocatedAdCostPart = line.split('分摊广告费:')[1].split('理论到手价:')[0].trim();
            const theoreticalPricePart = line.split('理论到手价:')[1].trim();
            info.allocatedAdCost = allocatedAdCostPart;
            info.theoreticalPrice = theoreticalPricePart;
        }
        // 平台佣金行 - 包含平台佣金和广告进项抵扣
        else if (line.includes('平台佣金:') && line.includes('广告进项抵扣:')) {
            const platformFeePart = line.split('平台佣金:')[1].split('广告进项抵扣:')[0].trim();
            const adVatCreditPart = line.split('广告进项抵扣:')[1].trim();
            info.platformFee = platformFeePart;
            info.adVatCredit = adVatCreditPart;
        }
        // 销项税行 - 包含销项税和平台进项抵扣
        else if (line.includes('销项税:') && line.includes('平台进项抵扣:')) {
            const outputVATPart = line.split('销项税:')[1].split('平台进项抵扣:')[0].trim();
            const platformVatCreditPart = line.split('平台进项抵扣:')[1].trim();
            info.outputVAT = outputVATPart;
            info.platformVatCredit = platformVatCreditPart;
            console.log('提取销项税:', outputVATPart, '平台进项抵扣:', platformVatCreditPart); // 添加调试信息
        }
        // 实缴增值税行 - 包含实缴增值税和总成本
        else if (line.includes('实缴增值税:') && line.includes('总成本:')) {
            const actualVATPart = line.split('实缴增值税:')[1].split('总成本:')[0].trim();
            const totalCostPart = line.split('总成本:')[1].trim();
            info.actualVAT = actualVATPart;
            info.totalCost = totalCostPart;
        }
    });
                ...
                <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.4; max-width: 600px;">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">
                        ...
                        <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                            <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">📊 基础参数</div>
                            <div style="font-size: 11px; color: #6b7280;">
                                目标利润率：${info.targetProfitRate || '—'}<br>
                                付费占比：${info.adRate || '—'}<br>
                                退货率：${info.returnRate || '—'}<br>
                                有效销售率：${info.effectiveRate || '—'}<br>
                                推演成本价：${info.costPrice || '—'}
                            </div>
                        </div>

                        <!-- 成本价计算 -->
                        <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                            <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">💰 成本价计算</div>
                            <div style="font-size: 11px; color: #6b7280;">
                                开票成本：${info.invoiceCost || '—'}<br>
                                总进货成本：${info.totalPurchaseCost || '—'}<br>
                                商品进项税：${info.purchaseVAT || '—'}<br>
                                固定成本：${info.fixedCosts || '—'}<br>
                                分摊后成本：${info.allocatedCosts || '—'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">
                        ...
                        <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                            ...
                            <div style="font-size: 11px; color: #6b7280;">
                                广告费：${info.adCost || '—'}<br>
                                分摊广告费：${info.allocatedAdCost || '—'}<br>
                                平台佣金：${info.platformFee || '—'}<br>
                                总成本：${info.totalCost || '—'}
                            </div>
                        </div>
                        ...
                        <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                            ...
                            <div style="font-size: 11px; color: #6b7280;">
                                计算利润：${info.profit || '—'}<br>
                                利润率：${info.profitRate || '—'}<br>
                                目标差异：${info.targetDiff || '—'}<br>
                                理论到手价：${info.theoreticalPrice || '—'}
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="background: #f8fafc; padding: 8px; border-radius: 4px; margin-bottom: 8px;">
                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">🧾 税费计算详情</div>
                        <div style="font-size: 11px; color: #6b7280;">
                            <div style="margin-bottom: 4px;">
                                <strong>销项税：</strong>${info.outputVAT || '—'}
                            </div>
                            <div style="margin-bottom: 4px;">
                                <strong>进项税抵扣：</strong><br>
                                &nbsp;&nbsp;• 商品进项税：${info.purchaseVAT || '—'}<br>
                                &nbsp;&nbsp;• 广告进项抵扣：${info.adVatCredit || '—'}<br>
                                &nbsp;&nbsp;• 平台进项抵扣：${info.platformVatCredit || '—'}
                            </div>
                            <div style="margin-bottom: 4px;">
                                <strong>实缴增值税：</strong>${info.actualVAT || '—'}${info.actualVAT && info.theoreticalPrice ? ` <small style="color: #059669;">(占到手价 ${((parseFloat(info.actualVAT.replace('¥', '')) / parseFloat(info.theoreticalPrice.replace('¥', ''))) * 100).toFixed(2)}%)</small>` : ''}<br>
                                <small style="color: #9ca3af;">(销项税 - 总进项抵扣)</small>
                            </div>
                        </div>
                    </div>
...
function generateCostPriceTableHtmlForExploration(results, adRates, targetProfitRates, returnRate, inputs) {
    ...
    return `
        <div style="background:#fff; border:1px solid #e5e7eb; border-radius:12px; overflow-x:auto; box-shadow:0 4px 6px -1px rgba(0,0,0,0.1);" class="takehome-table-container">
            <table style="width:100%; min-width:600px; border-collapse:collapse; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;" class="takehome-result-table" id="costExplorationTable">
                ${header}
                ${rows}
            </table>
        </div>
 ...
 * @param {string} basicInfo 基本信息
 * @param {string} calculationDataJson 计算数据JSON字符串
 */
function showTakeHomeCalculationTooltip(event, basicInfo, calculationDataJson) {
    // 移除已存在的浮窗
    hideTakeHomeCalculationTooltip();
    ...
    try {
        ...
        // 使用统一的利润计算函数，确保与推演表格结果完全一致
        const inputs = {
            costPrice: data.costPrice,
            actualPrice: data.takeHomePrice,
        ...
        const otherCost = data.otherCost;
        const returnRate = data.returnRate;
        const adRate = data.adRate;
        const targetRate = data.targetRate;
        const takeHomePrice = data.takeHomePrice;

        // 使用统一函数的结果
        const invoiceCost = result.effectiveCost - costPrice; // 开票成本
        const actualPurchaseCost = result.effectiveCost; // 实际进货成本
        const inputTaxCredit = result.purchaseVAT; // 进项税抵扣

        const effectiveRate = 1 - returnRate; // 有效销售率
        const nonRefundableCosts = result.shippingCostEffective + result.insuranceCostEffective + result.otherCostEffective; // 非退款成本分摊
        const advertisingCost = result.adCostEffective * effectiveRate; // 广告费用（还原为原始值）
        ...
        const calculationHtml = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.4; max-width: 600px;">
                <div style="font-weight: 700; color: #1f2937; margin-bottom: 10px; font-size: 15px;">🧮 详细计算过程</div>

                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">
                    <!-- 基础数据 -->
                    <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">📊 基础数据</div>
                        <div style="font-size: 11px; color: #6b7280;">
                            进货价：¥${costPrice.toFixed(2)}<br>
                            开票成本：¥${invoiceCost.toFixed(2)} (${(inputTaxRate * 100).toFixed(1)}%)<br>
                            实际成本：¥${actualPurchaseCost.toFixed(2)}<br>
                            进项抵扣：¥${inputTaxCredit.toFixed(2)} (${(outputTaxRate * 100).toFixed(1)}%)<br>
                            到手价：¥${takeHomePrice.toFixed(2)}
                        </div>
                    </div>
                    ...
                    <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                        ...
                        <div style="font-size: 11px; color: #6b7280;">
                            ...
                            物流分摊：¥${(shippingCost / effectiveRate).toFixed(2)}<br>
                            运费险分摊：¥${(shippingInsurance / effectiveRate).toFixed(2)}<br>
                            其他分摊：¥${(otherCost / effectiveRate).toFixed(2)}<br>
                            广告费：¥${advertisingCost.toFixed(2)}<br>
                            平台佣金：¥${platformCommission.toFixed(2)}
                        </div>
                    </div>
                </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 8px;">
                    <!-- 税费计算 -->
                    <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">🧾 税费计算</div>
                        <div style="font-size: 11px; color: #6b7280;">
                            销项税：¥${outputTax.toFixed(2)}<br>
                            广告进项税：¥${advertisingTaxCredit.toFixed(2)}<br>
                            佣金进项税：¥${commissionTaxCredit.toFixed(2)}<br>
                            总进项抵扣：¥${totalTaxCredits.toFixed(2)}<br>
                    ...
                    <!-- 利润计算 -->
                    <div style="background: #f8fafc; padding: 8px; border-radius: 4px;">
                        <div style="font-weight: 600; color: #374151; margin-bottom: 4px; font-size: 12px;">📈 利润计算</div>
                        <div style="font-size: 11px; color: #6b7280;">
                            总成本：¥${totalCost.toFixed(2)}<br>
                            实际利润：¥${actualProfit.toFixed(2)}<br>
                            实际利润率：${actualProfitRate.toFixed(2)}%<br>
                            目标利润率：${(targetRate * 100).toFixed(1)}%
                        </div>
                    </div>
                </div>
                ...
                <div style="background: #e0f2fe; padding: 8px; border-radius: 4px; border-left: 3px solid #0ea5e9;">
                    <div style="font-size: 11px; color: #0c4a6e; line-height: 1.3;">
                        <div style="font-weight: 600; margin-bottom: 2px;">💡 计算公式：</div>
                        <div>• 有效销售率 = 1 - 退货率</div>
                        <div>• 非退款成本分摊 = 原成本 ÷ 有效销售率</div>
                        <div>• 利润率 = (到手价 - 总成本) ÷ 到手价</div>
                    </div>
                </div>
                ...
                <div style="background: #d1fae5; padding: 8px; border-radius: 4px; border-left: 3px solid #10b981; margin-top: 8px;">
                    <div style="font-size: 11px; color: #065f46; line-height: 1.3;">
                        <div style="font-weight: 600; margin-bottom: 2px;">✅ 计算一致性说明：</div>
                        <div>• 推演表格使用与售价计算tab完全相同的精确公式</div>
                        <div>• 无迭代计算，直接求解，结果精确</div>
                        <div>• 浮窗显示的是基于最终到手价的正向验证计算</div>
                        <div>• 两者数据完全一致，无精度差异</div>
                    </div>
                </div>
            </div>
        `;
        ...
        // 创建浮窗元素
        const tooltip = document.createElement('div');
        tooltip.id = 'takehome-calculation-tooltip';
        ...
        const handleMouseMove = (e) => {
            ...
            const adjustedLeft = newLeft + 480 > viewportWidth ? e.clientX - 480 - offset : newLeft;
            const adjustedTop = newTop + tooltip.offsetHeight > viewportHeight ? e.clientY - tooltip.offsetHeight - offset : newTop;
        ...
        event.target._tooltipMoveHandler = handleMouseMove;
        
        // 添加全局鼠标移动监听器，当鼠标离开表格区域时自动隐藏浮窗
        ...
        event.target._globalMouseMoveHandler = globalMouseMoveHandler;

    } catch (error) {
        console.error('显示计算过程浮窗失败:', error);
    }
}
 ...
 * 隐藏到手价推演详细计算过程浮窗
 */
function hideTakeHomeCalculationTooltip() {
    const tooltip = document.getElementById('takehome-calculation-tooltip');
    if (tooltip) {
        // 移除事件监听器
        const cells = document.querySelectorAll('td[data-calculation]');
        cells.forEach(cell => {
            if (cell._tooltipMoveHandler) {
                cell.removeEventListener('mousemove', cell._tooltipMoveHandler);
                delete cell._tooltipMoveHandler;
            }
            if (cell._globalMouseMoveHandler) {
                document.removeEventListener('mousemove', cell._globalMouseMoveHandler);
                delete cell._globalMouseMoveHandler;
            }
        });
